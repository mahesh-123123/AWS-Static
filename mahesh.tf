...
resource "aws_s3_bucket" "mahesh-bucket" {
  bucket = "mahesh-static-data-bucket"
  acl    = "public-read"
}

//Putting Objects in S3 Bucket
resource "aws_s3_bucket_object" "web-object1" {
  bucket = "${aws_s3_bucket.mahesh-bucket.bucket}"
  key    = "mahesh.png"
  source = "C:/terraform/maheshreddy/mahesh.png"
  acl    = "public-read"
}
